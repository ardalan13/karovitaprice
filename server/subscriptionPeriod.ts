 ...
 * @param days تعداد روز
 */
export function getRawSubscriptionMonths(days: number | string): number {
 ...
 * @param days تعداد روزهای باقیمانده یا مدت پکیج
 * @returns تعداد ماه‌ها به صورت عدد صحیح
 */
export function calculateSubscriptionMonths(days: number | string): number {
  const d = Math.max(0, Number(days) || 0);
