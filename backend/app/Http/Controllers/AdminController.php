...
class AdminController extends Controller {
    ...
    public function users() {
        $users = User::with('company')->latest()->paginate(50);
        return response()->json([
            'data' => $users->items(),
            'pagination' => [
    ...
    public function tickets() {
        $tickets = Ticket::with([
                'department',
                'user',
                'messages' => fn($q) => $q->latest()->limit(1) // Only last message
            ])
            ->latest()
            ->paginate(30);
        return response()->json([
    ...
    public function updateOrderStatus(Request $request, $id) {
        ...
        try {
            ...
            if ($status === 'paid' || $status === 'completed') {
                ...
                $existingSub = Subscription::where('order_id', $order->id)->first();
            ...
            AuditLogger::logAdminAction(
                ...
                "تغییر وضعیت سفارش #{$order->order_number} از {$previousStatus} به {$status}",
                ['order_id' => $order->id, 'old_status' => $previousStatus, 'new_status' => $status],
                $request->user()
            );
            ...
            return response()->json(['error' => 'خطا در بروزرسانی وضعیت سفارش'], 500);
        }
    }
