...
class AuthController extends Controller {
    ...
    public function verifyOtp(VerifyOtpRequest $request) {
        ...
        // Lock for update to prevent race conditions and check attempts
        ...
        if (!$otp) {
            ...
            AuditLog::create([
                ...
                'action_description' => "تلاش ناموفق برای ورود با شماره {$mobile}",
        ...
        // Get admin mobiles from environment
        $adminMobiles = array_filter(array_map('trim', explode(',', env('ADMIN_MOBILES', '09111273476'))));
        ...
        $user = User::firstOrCreate(
            ['mobile' => $mobile],
            [
                'name' => $isAdmin ? 'مدیر سیستم' : ('کاربر ' . substr($mobile, -4)),
        ...
        // Generate secure 64 char token - ALWAYS set expires_at
        $tokenString = Str::random(64);
        AuthToken::create([
            'user_id' => $user->id,
            'token' => $tokenString,
            'expires_at' => Carbon::now()->addDays(30), // Always set expiry
        ]);
