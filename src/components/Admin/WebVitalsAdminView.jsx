                ...
                {vitals.map((item) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.15s' }}>
                    ...
                    <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      {getMetricBadge(item?.metrics?.lcp ?? (item?.name === 'LCP' ? item?.value : null), 'lcp')}
                    </td>

                    <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      {getMetricBadge(item?.metrics?.cls ?? (item?.name === 'CLS' ? item?.value : null), 'cls')}
                    </td>

                    <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      {getMetricBadge(item?.metrics?.inp || item?.metrics?.fid || (item?.name === 'INP' || item?.name === 'FID' ? item?.value : null), 'inp')}
                    </td>

                    <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      {getMetricBadge(item?.metrics?.fcp ?? (item?.name === 'FCP' ? item?.value : null), 'fcp')}
                    </td>

                    <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      {getMetricBadge(item?.metrics?.ttfb ?? (item?.name === 'TTFB' ? item?.value : null), 'ttfb')}
                    </td>
