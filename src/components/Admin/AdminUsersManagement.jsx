...
export function AdminUsersManagement({ data = [], reload }) {
  ...
  async function handleDeleteUser(user) {
    if (user.is_owner || user.mobile === '09111273476' || user.id === 1) {
      alert('امکان حذف حساب مالک اصلی و مدیر ارشد سامانه وجود ندارد.');
      return;
    }

    const userName = [user.first_name, user.last_name].filter(Boolean).join(' ') || user.mobile;
    if (!confirm(`آیا از حذف کامل و دائمی حساب کاربر «${userName}» (${user.mobile}) اطمینان دارید؟\n\nتوجه: با این اقدام، کلیه داده‌های کاربر (پروفایل، اطلاعات شرکت، اشتراک‌ها، فاکتورها، تیکت‌ها و نشست‌های ورود) به طور کامل و غیرقابل‌بازگشت پاک می‌شوند و کاربر در مراجعات بعدی می‌تواند از مرحله ۱ به عنوان کاربر کاملاً جدید ثبت‌نام نماید.`)) {
      return;
    }

    try {
      const res = await api(`/admin/users/${user.id}`, { method: 'DELETE' });
      setSuccessMsg(res.message || `کلیه اطلاعات کاربر ${userName} با موفقیت و به صورت دائمی پاک شد.`);
      if (reload) reload();
    } catch (err) {
