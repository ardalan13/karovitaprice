...
/**
 * Save/Update user role ('admin' | 'user') in Cookie and LocalStorage
 */
export function saveStoredRole(role) {
  ...
  try {
    ...
    console.warn('Failed to set role cookie', err);
  }
  ...
  try {
    ...
    console.warn('Failed to set role in localStorage', err);
  }
}
