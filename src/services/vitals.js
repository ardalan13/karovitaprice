...
export function initPerformanceMonitoring(options = {}) {
  ...
  const flushIntervalMs = options.intervalMs || 300000; // 5 minutes default (prevents host rate-limit issues)
  ...
  try {
    ...
    if ('PerformanceObserver' in window && PerformanceObserver.supportedEntryTypes?.includes('first-input')) {
      const fidObserver = new PerformanceObserver((entryList) => {
        try {
          for (const entry of entryList.getEntries()) {
            if (entry && typeof entry.processingStart === 'number' && typeof entry.startTime === 'number') {
    ...
    if ('PerformanceObserver' in window && PerformanceObserver.supportedEntryTypes?.includes('event')) {
      ...
      try {
        const inpObserver = new PerformanceObserver((entryList) => {
          try {
            for (const entry of entryList.getEntries()) {
              if (entry && typeof entry.duration === 'number' && entry.duration > maxInteractionDuration) {
    ...
    // Send initial snapshot after 5 seconds once metrics settle
    initialTimeoutTimer = setTimeout(() => {
      collectNavigationTiming();
      flushVitalsReport();
    }, 5000);

    // Setup periodic flush interval (5 minutes)
    flushIntervalTimer = setInterval(() => {
      if (hasNewData && navigator.onLine) {
        flushVitalsReport();
      }
    }, flushIntervalMs);

    // Flush on page visibility change with a 1-minute minimum cooldown
    let lastVisibilityFlush = 0;
    const throttledFlush = () => {
      const now = Date.now();
      if (now - lastVisibilityFlush < 60000) return;
      if (hasNewData && navigator.onLine) {
...
export function flushVitalsReport() {
  ...
  const now = Date.now();
  if (now - lastReportTime < 30000) return; // Cooldown: at most once every 30s
  lastReportTime = now;
